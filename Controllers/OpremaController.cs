        ...
        public IActionResult Delete(string Naziv, int page = 1, int sort = 1, bool ascending = true)
        {
            ...
            if (oprema != null)
            {
                try
                {
                    ...
                    ctx.Remove(oprema);
                    ctx.SaveChanges();
                    logger.LogInformation($"Opream {naziv} uspješno obrisana");
                    TempData[Constants.Message] = $"Oprema {naziv} uspješno obrisana";
                    TempData[Constants.ErrorOccurred] = false;
                }
                catch (Exception exc)
                {
            ...
            {
                logger.LogWarning("Ne postoji opremea s oznakom: {0} ", Naziv);
                TempData[Constants.Message] = "Ne postoji oprema s oznakom: " + Naziv;
                TempData[Constants.ErrorOccurred] = true;
            }
